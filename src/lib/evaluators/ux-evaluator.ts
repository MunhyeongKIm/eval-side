...
function evaluateUXFallback(input: EvaluationInput): ScoreResult {
  ...
  if (input.type === 'github' && input.repoAnalysis) {
    ...
    if (hasDemo) { score += 3; strengths.push('Live demo/deployment URL present'); }
    else { score += 1; improvements.push('Adding a live demo URL is recommended'); }
    ...
    score += Math.min(matched.length * 2 + 2, 10);
  ...
  return { score, maxScore: 10, grade, analysis: `UX analysis complete. Score: ${score}/10`, strengths, improvements };
}
